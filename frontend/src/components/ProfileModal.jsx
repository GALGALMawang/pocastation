...
function ProfileModal({ user, onClose, fetchExtra }) {
  ...
  const handleSave = async () => {
    setLoading(true);
    const { error } = await supabase.from('profiles').update({ phone, bank_account: bank }).eq('id', user.id);
    setLoading(false);
    if (error) alert(error.message);
    else { alert('설정이 저장되었습니다.'); fetchExtra(); onClose(); }
  };

  return (
    <div className="mod-wrap open"><div className="mod-ov"><div className="modal" style={{maxWidth:'400px'}}>
      <div className="mod-hd"><div className="mod-ttl">내 프로필 & 알림 설정</div><button className="mod-cl" onClick={onClose}>×</button></div>
      <div className="mod-body">
        ...
        <div className="form-grp">
          ...
          <input type="text" value={bank} onChange={e=>setBank(e.target.value)} placeholder="은행명 계좌번호 예금주" />
        </div>
