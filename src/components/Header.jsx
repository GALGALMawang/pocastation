/**
 * Header.jsx — 상단 내비게이션 바
 *
 * Props:
 *   user        - Supabase 유저 객체 (null이면 비로그인)
 *   profile     - profiles 테이블 행 (닉네임 등)
 *   credit      - 현재 크레딧 잔액
 *   onOpenModal - 모달 키를 받아 해당 모달 오픈: 'auth' | 'charge' | 'create' | 'bids' | 'profile'
 *   activeView  - 현재 활성 뷰: 'live' | 'ended' | 'artist'
 *   onNavClick  - 네비 링크 클릭 콜백 (view 문자열 전달)
 *   search      - 현재 검색 키워드
 *   onSearch    - 검색 키워드 변경 콜백
 */
    ...
    <header className="hdr">
      <div className="pg hdr-in">
        ...
        <a href="#" className="logo">
          ...
          <div>
            <div className="logo-nm">POCA</div>
            <div className="logo-ds">K-POP 포카 경매</div>
          </div>
        </a>

        {/* 검색창 — 입력 시 경매 섹션으로 자동 스크롤 */}
        <div className="srch">
        ...
        {/* 네비게이션 — activeView 기반으로 활성 링크 표시 */}
        <nav className="gnv">
          <a
            href="#auctions"
            className={activeView === 'live' ? 'on' : ''}
            onClick={e => { e.preventDefault(); onNavClick('live'); }}
          >경매</a>
          <a
            href="#auctions"
            className={activeView === 'artist' ? 'on' : ''}
            onClick={e => { e.preventDefault(); onNavClick('artist'); }}
          >아티스트</a>
          <a
            href="#auctions"
            className={activeView === 'ended' ? 'on' : ''}
            onClick={e => { e.preventDefault(); onNavClick('ended'); }}
          >종료된 경매</a>
        </nav>
